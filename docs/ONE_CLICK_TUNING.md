...
- **FERR watchdog.** A 1 kHz sampler watches drive 60F4 during every move
  ...
  (0.5 mm / 0.5° production window → abort at 0.8 × window; one-click raises 6065
  to 2.0 during moves). An unstable gain step is
...
| Watchdog trips at the *first* speed step | Baseline is closer to instability than it looks, or 6065 window is tight | `tripped_ferr` value in the measure meta | verify 6065 (production **0.5 mm**; tuning raises to **2.0**), start from a softer baseline preset |

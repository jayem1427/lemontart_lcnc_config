...
- **FERR watchdog.** A 1 kHz sampler watches drive 60F4 during every move
  ...
  (production **0.5 mm / 0.5°** on this machine → abort at 0.4; during a run
  6065 is temporarily **2.0** so the watchdog threshold scales accordingly). An
  unstable gain step is
...
| Watchdog trips at the *first* speed step | Baseline is closer to instability than it looks, or 6065 window is tight | `tripped_ferr` value in the measure meta | verify 6065 (production **0.5 mm / 0.5°**; tuning raises to **2.0** during moves), start from a softer baseline preset |
